...
--r57 Added missing settings
if TUPPMSettings.mtbs_ENABLE_lethalWeaponsOnMB then
	--r38 Allow usable weapons(AA guns, mortars, heli minigun) on Zoo
	PlayerDisableActionFlagDefault = PlayerDisableAction.NONE
	PlayerDisableActionFlagInBirdCage = PlayerDisableAction.FULTON
end
...
function this.OnEndMissionPrepareSequence()
	if not TUPPMSettings.mtbs_ENABLE_lethalWeaponsOnMB then
		--r38 Allow killing weapons on Zoo
...
function this.OnRestoreSVars()
	...
	this.ShowGotAnimals() --rX51 entry point
...
local messagesTable = StrCode32Table {
	...
	GameObject = {
		...
		{ --r38 Demon transformation option
		  msg = "Dead",
      func = function(gameObjectId)
	      --r57 Added missing settings
    ...
    { --r38 Demon transformation option
      ...
      func = function(gameObjectId)
      	--r57 Added missing settings
      	if not TUPPMSettings.player_ENABLE_demonPointsManipViaZoo then return end
				--TppPlayer.TUPPMPrint("Awww you sweety :*")
				--TppHero.SetOgrePoint(-1*TppMotherBaseManagement.GetOgrePoint())
        
        if (TppMotherBaseManagement.GetOgrePoint()~=0) then
          TppMusicManager.PostJingleEvent("SingleShot","Play_bgm_common_jingle_achieved") --Good :) - objectives achieved
        end
        TppMotherBaseManagement.SubOgrePoint{ogrePoint=TppMotherBaseManagement.GetOgrePoint()}
      end,
    },
...
function this.Messages()
	...
	this.SetRegisteredVisibleAnimalTable() --rX51 visible animals
