...
local sequences = {}
--r48 More staff on MB platforms, 9 per platform
--r51 Settings
local MAX_STAFF_NUM_IN_CLUSTER = math.floor(math.min(math.max(TUPPMSettings.mtbs_totalStaffCountOnEachMBCluster or 18,18),36))
--r51 Settings
if not TUPPMSettings.mtbs_ENABLE_extraSoldiersOnMB then
	MAX_STAFF_NUM_IN_CLUSTER = 18
end
...
--r51 Settings
if TUPPMSettings.mtbs_ENABLE_lethalWeaponsOnMB then
	--r38 Enable Kill actions on MB
...
function this.SetupStaffList()
	...
	local MAX_STAFF_NUM_ON_CLUSTER = MAX_STAFF_NUM_IN_CLUSTER
	local staffList = {}
	...
	local _SetupStaffIdOnCluster = function(staffListOnCluster, clusterIndex)
	  --r38 More female staff on platforms
	  --r51 Settings
	  local femaleStaffCountRandom = 2
		...
		do 
			...
			for i = 1, staffNum do
				...
				if this.IsFemale( staffId ) then
					...
					if #mvars.f30050_staffIdList[clusterIndex+1] >= femaleStaffCountRandom then
...
function this.OnEndMissionPrepareSequence()
	...
	--r51 Settings
	if not TUPPMSettings.mtbs_ENABLE_lethalWeaponsOnMB then
		--r38 Enable Kill weapons equip menu
...
local MAX_FACE_NUM_IN_CLUSTER = MAX_STAFF_NUM_IN_CLUSTER                                                          
...
--	TppEnemy.MB_SET_FEMALE_ALL_STAFF=true --rX7 --activates an army of clones
...
function this.Messages()
	...
	StrCode32Table {
		...
		GameObject = {
...

			{
				...
				func = function(gameObjectId)
					if not Tpp.IsPlayer(gameObjectId) then
						...
						--r38 Can disable mission failure on DD kill here
			...
			{ --r38 Let's not go crazy and destroy the support heli as if it's okay
        msg="LostControl",
        func=function(gameId,state,attackerId)
          TppMission.ReserveGameOver( TppDefine.GAME_OVER_TYPE.HELICOPTER_DESTROYED, TppDefine.GAME_OVER_RADIO.PLAYER_KILL_DD )
--          TppMission.ReserveGameOver( TppDefine.GAME_OVER_TYPE.HELICOPTER_DESTROYED, TppDefine.GAME_OVER_RADIO.PLAYER_DESTROY_HELI ) --missions a failure
--          TppMission.ReserveGameOver( TppDefine.GAME_OVER_TYPE.HELICOPTER_DESTROYED, TppDefine.GAME_OVER_RADIO.HELICOPTER_DESTROYED ) --chopper went down, no extraction
        end
      },
		},
		Trap = {
...
this.SetDDogArea = function(  )
	if vars.buddyType == BuddyType.DOG then
		...
		--r51 Settings
		local radius = math.floor(math.min(math.max(TUPPMSettings.mtbs_dDogMovementRadius or 45,45),3000))
		--r38 D-Dog can follow you all over MB
		--Commenting out the call still sets a radius which seems to be ~50
...
sequences.Seq_Game_MainGame = {
				...
				},
				{
					...
					func = function( trapName, gameObjectId )
						if Tpp.IsSoldier( gameObjectId ) then 
							...
							--r55 Not sure about this whether this is needed or not but may help
							if not TUPPMSettings.routes_ENABLE_shuffle then
	...
	OnEnter = function()
		...
		--r45 Setup salute soldiers when starting MB even on foot
		--r55 Re added cause on a new MB shit gets out of hand if soldiers from other plat arrive to salute on a single command cluster
	  if 
	  	TppMain.firstFakeHeli==1 
	  	and not TppMain.comingFromTitleDontFireHeliRemoval --not when coming from title
	  	and not TppMain.mbVeryFastCheckpointReloadHeliRouteNotFireFIX --not when reloading checkpoint
...
function this.EnableAssetsOnCluster(clusterId)
	...
	--r51 Settings
	if not TUPPMSettings.mtbs_ENABLE_unlockFOBGoalDoors then
		--r46 Doors no longer locked. Yay! :)
...
--	TUPPMLog.Log("f30050_sequence.EnableBiggBossPosters which means forced Mission.LoadLocation working",3)
	
	--r51 Settings
	local randomVar = 0
	if TUPPMSettings.mtbs_ENABLE_bbPostersOnMB then
		TppMain.Randomize()
		randomVar = math.random()
		TUPPMLog.Log("EnableBiggBossPosters randomVar:"..tostring(randomVar),3)
		
		--r46 Hide when checkpoints are reloaded - MB has only one checkpoint anyway :)
		-- This makes this in sync with soldiers being different on every checkpoint reload
		TppDataUtility.SetVisibleDataFromIdentifier( "mtbs_plant0", "bwy00", false, false)
		TppDataUtility.SetVisibleDataFromIdentifier( "mtbs_plant0", "bwy01", false, false)
		TppDataUtility.SetVisibleDataFromIdentifier( "mtbs_plant1", "bwy00", false, false)
		TppDataUtility.SetVisibleDataFromIdentifier( "mtbs_plant2", "bwy00", false, false)
		TppDataUtility.SetVisibleDataFromIdentifier( "mtbs_plant3", "bwy00", false, false)
	end
	
	if 	
		(TppDefine.STORY_SEQUENCE.CLEARD_OKB_ZERO <= TppStory.GetCurrentStorySequence()
			and not TppStory.IsMissionCleard( 10240 ))
		--r46 Enable BB posters sometimes
		or
		(TUPPMSettings.mtbs_ENABLE_bbPostersOnMB and TppStory.IsMissionCleard( 10240 ) and randomVar<math.min(math.max(TUPPMSettings.mtbs_bbPostersChance or 0.15,0),1))
	then
...
function this.SetupBattleHanger()
	...
	TppUI.OverrideFadeInGameStatus{ --TODO rX46 nothing usefel sadly --Look for this too
		EquipHud = false,
		EquipPanel = false,
	}
	this.SetEnableQuestUI(false)
