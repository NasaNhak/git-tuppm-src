...
mbdvc_map_location_parameter = {
	...
	GetGlobalLocationParameter = function()
		...
		--r51 Settings
		local function getSpyRadiusARank()
			if TUPPMSettings.game_ENABLE_realTimeSpySearch then
				return 5.0
			else
				return 15.0
			end
		end
		local function getSpyIntervalARank()
			if TUPPMSettings.game_ENABLE_realTimeSpySearch then
				return 30.0
			else
				return 120.0
		...
		local function getSpyRadiusSRank()
			if TUPPMSettings.game_ENABLE_realTimeSpySearch then
				return 0.0
			else
				return 10.0
		...
		local function getSpyIntervalSRank()
			if TUPPMSettings.game_ENABLE_realTimeSpySearch then
				return 0.0
			else
				return 60.0
		...
		return {
			{	
				...
				--r20 Soldier movement fields are highly accurate and updated in real time with A Rank in Scouting Unit Function for Intel Unit
				spySearchRadiusMeter = {	40.0,	40.0,	35.0,	30.0,	25.0,	20.0,	getSpyRadiusARank(),	getSpyRadiusSRank(), }, --r20 revising for auto marking --r18 very accurate
				spySearchIntervalSec = {	420.0,	420.0,	360.0,	300.0,	240.0,	180.0,	getSpyIntervalARank(),	getSpyIntervalSRank(), }, --r20 revising for auto marking --r18 real time update
