...
sequences.Seq_Demo_WaitCopyRightLogo = {
	OnUpdate = function(self)
		...
		if not screen then
			...
			--r51 Settings
			if not TUPPMSettings.startup_ENABLE_logosSkip then
...
sequences.Seq_Demo_ConfirmAutoSave = {
	OnEnter = function( self )
		...
		if SignIn.PresetUserIdExists and SignIn.PresetUserIdExists() then
			...
			--r13 Below sequence can be used as well
			--TppSequence.SetNextSequence("Seq_Demo_CreateOrLoadSaveData")
			return
		end

		--r51 Settings
		if not TUPPMSettings.startup_ENABLE_autoSavePopUpSkip then
			...
			TppSequence.SetNextSequence("Seq_Demo_StartSignIn") --K Skip Auto-Saving confirm
			--r13 Below sequence can be used as well
	--		TppSequence.SetNextSequence("Seq_Demo_CreateOrLoadSaveData") --K Skip Auto-Saving confirm
		end
	end,
...
--r65 Re-attempt login if failed
local loginSequencePopupTime = 0
local currentLoginAttempts=0
local maxLoginAttempts = TUPPMSettings.startup_maxReloginAttempts or 0
sequences.Seq_Demo_LogInKonamiServer = {
			...
			Network = {
				{
					...
					func = function()
						...
						if 
							...
							and currentLoginAttempts < maxLoginAttempts
							and (TppGameMode.GetUserMode()<=TppGameMode.U_1ST_PARTY_SIGN_IN) 
						then
							--TppUiCommand.ErasePopup() --NOPE this message is passed after the popup is displayed --automatically removes popup :)
	...
	OnEnter = function( self )
		...
		--r51 Settings
		if not TUPPMSettings.startup_ENABLE_loginSkip then 
			TppServerManager.StartLogin() --login function while startup
		else
			--r13 Below sequence is used after login
			TppSequence.SetNextSequence("Seq_Demo_CheckPatchDlcForInvitation")
		end
	end,
	...
	OnUpdate = function ( self )
		...
		then
			...
			loginSequencePopupTime = loginSequencePopupTime + Time.GetFrameTime()
		end
	end,
}
