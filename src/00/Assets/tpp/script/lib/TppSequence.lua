...
local TppScriptVarsSVarsIsSynchronized=TppScriptVars.SVarsIsSynchronized
...
function this.SetNextSequence(sequenceName,options)
	--TUPPMLog.Log("TppSequence.SetNextSequence START sequenceName:"..tostring(sequenceName),1)
	local sequenceNumber=nil
	if mvars.seq_sequenceNames then
		sequenceNumber=mvars.seq_sequenceNames[sequenceName]
	end
	if sequenceNumber==nil then
		return
	end
	...
	if options and IsTypeTable(options)then
		isExecMissionClear=options.isExecMissionClear
		isExecGameOver=options.isExecGameOver
		isExecDemoPlaying=options.isExecDemoPlaying
		isExecMissionPrepare=options.isExecMissionPrepare
	end
	if TppMission.CheckMissionState(isExecMissionClear,isExecGameOver,isExecDemoPlaying,isExecMissionPrepare)then
		svars.seq_sequence=sequenceNumber
		--TUPPMLog.Log("TppSequence.SetNextSequence END returning",1)
		return
	end
	--TUPPMLog.Log("TppSequence.SetNextSequence END non-returning",1)
end
function this.ReserveNextSequence(sequenceName,options)
	--TUPPMLog.Log("TppSequence.ReserveNextSequence START sequenceName:"..tostring(sequenceName),1)
	TppScriptVars.SetSVarsNotificationEnabled(false)
	this.SetNextSequence(sequenceName,options)
...
return""end
local a=this.GetSequenceNameWithIndex
...
function this.MakeSVarsTable(n)
local s={}
local e,t,t=1
...
s[e]={name=i,type=TppScriptVars.TYPE_UINT32,value=StrCode32(n),save=true,sync=true,category=TppScriptVars.CATEGORY_MISSION}
elseif t=="table"then
s[e]=n
end
e=e+1
		...
		return Tpp.StrCode32Table{
			UI={
				...
				{msg="StartMissionTelopFadeIn",
					func=function()
						GkEventTimerManagerStart("Timer_HelicopterMoveStart",s)
					end,
					option={isExecMissionPrepare=true,isExecMissionClear=true,isExecGameOver=true}},
				...
				{msg="PushEndLoadingTips",
					func=function()
						...
						GkEventTimerManagerStart("Timer_WaitStartingGame",1)
					end,
					option={isExecMissionPrepare=true,isExecMissionClear=true,isExecGameOver=true}}
			},
			Timer={
		...
		if mvars.seq_demoSequneceList[mvars.seq_missionStartSequence]then
			...
			if mvars.seq_isHelicopterStart then
				if mvars.seq_noMissionTelopOnHelicopter then
					...
					GkEventTimerManagerStart("Timer_FadeInStartOnNoTelopHelicopter",t)
				else
	...
	DEBUG_TextPrint=function(n)
		...
		if(mvars.seq_missionPrepareState<this.MISSION_PREPARE_STATE.END_SAVING_FILE)then
			...
			if TppUiCommand.IsEndLoadingTips()then
				TppUI.FinishLoadingTips()GkEventTimerManagerStart("Timer_WaitStartingGame",o)
			else
...
function this.OnMessage(o,s,a,i,r,n,t)
